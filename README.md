...
Run `sudo dnf upgrade` to include updates to `normlang-release`, which distributes repository configuration and the public key. Configuration or key changes require a new `normlang-release` package version or release. Public-key renewal uses an explicit reviewed publication; recovery for clients that miss a key transition has not yet been validated.
